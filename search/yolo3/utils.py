...
import random
import colorsys
from search.yolo3.yolov3 import *
from searchImages.settings import YOLO_V3_WEIGHTS
...
def detect_image(Yolo, image, input_size=416, score_threshold=0.3,
                 iou_threshold=0.45):
    ...
    return {'segments': list(map(lambda x: {"boxes": x[:4], "scores": x[4], "classes": str(int(x[5]))},
                                 np.array(bboxes).tolist())), 'width': image_w, 'height': image_h}